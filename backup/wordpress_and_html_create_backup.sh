...
WEBSITE_DOMAIN="silkroademart.com"
...
# Backup PostgreSQL database and specific folders
log_message "Starting ${WEBSITE_DOMAIN} specific backup"

# Create temporary directory for combined backup
TEMP_BACKUP_DIR="${BACKUP_DIR}/${TEMP_BACKUP_PREFIX}_${TIMESTAMP}"
mkdir -p "${TEMP_BACKUP_DIR}" || error_exit "Failed to create temporary backup directory"

# Backup PostgreSQL database
log_message "Backing up PostgreSQL database ${DB_NAME}"
PGPASSWORD="${DB_PASSWORD}" pg_dump -U "${DB_USER}" -F c "${DB_NAME}" -f "${TEMP_BACKUP_DIR}/${DB_NAME}_${TIMESTAMP}.dump" || \
    error_exit "PostgreSQL database backup failed"

# Backup specific folders
log_message "Backing up ${WEBSITE_DOMAIN} folders"
cp -r "${WEBSITE_PATH}/data" "${TEMP_BACKUP_DIR}/" || error_exit "Failed to copy data directory"
cp -r "${WEBSITE_PATH}/public" "${TEMP_BACKUP_DIR}/" || error_exit "Failed to copy public directory"

# Create final zip archive
log_message "Creating final zip archive"
pushd "${TEMP_BACKUP_DIR}" > /dev/null || error_exit "Cannot change to temp backup directory"
zip -r "${BACKUP_DIR}/${WEBSITE_DOMAIN}_full_backup_${TIMESTAMP}.zip" . || error_exit "Failed to create zip archive"
popd > /dev/null

# Cleanup temporary directory
rm -rf "${TEMP_BACKUP_DIR}"

log_message "${WEBSITE_DOMAIN} specific backup completed"

# Create inventory (Currently commented out)
